...
const MatchCard = ({
  ...
  const { data: timeline, isLoading } = api.riot.getMatchTimelineByGameId.useQuery(
  ...
  const blueSide = match.info.participants.filter((team) => team.teamId === 100);
  const redSide = match.info.participants.filter((team) => team.teamId === 200);

  return (
    <div
      className="flex flex-col gap-3 rounded-lg border border-slate-600/30 px-8 py-2 transition-colors duration-200 hover:opacity-90 hover:cursor-pointer"
      style={{
        borderLeftWidth: "1px",
